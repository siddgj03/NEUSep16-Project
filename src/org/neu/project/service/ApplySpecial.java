...
public class ApplySpecial implements ApplySpecialInterface {
	...
	public void setSplTree() throws ParseException {
		if (f == 1) {
			...
			car.getSplTree().add(new CarSpecificSpecialList(spl, specialledPrice, spl.getSpecialEndDate()));
			car.getSplTreeByExpiry().add(new CarSpecificSpecialList(spl, specialledPrice, spl.getSpecialEndDate()));
		}
	}

	boolean checkForSpecialsUpdates() throws ParseException {
		boolean updated = false;
		for (CarSpecificSpecialList csse : car.getSplTreeByExpiry()) {
			DateFormat df = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
			Date today;
			today = (df.parse(df.format((new Date()))));
			if (df.parse(df.format((csse.getExpiry()))).after(today))
				updated = true;
		}
		return updated;
	}
	...
	public double getLowestPrice() throws ParseException {
		...
		if (checkForSpecialsUpdates()) {
			for (CarSpecificSpecialList css : car.getSplTree()) {
				lowest = css.getSpecialledPrice();
				break;
			}
