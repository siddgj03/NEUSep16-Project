...
public class SimilarPanel extends DetailViewBasePanel {
	...
	private Component getSimilarPanel() {
		...
		JLabel saleValue = new JLabel(String.valueOf(vehicle.getPrice()));
		panel.add(sale);
		panel.add(saleValue);
	...
	private List<Vehicle> getSimilarVehicleList() {
		...
		while (index < vehicleList.size()) {
			if (vehicleList.get(index).getPrice() - 1000 < selectedVehicle.getPrice()
					&& vehicleList.get(index).getPrice() + 1000 > selectedVehicle.getPrice()
